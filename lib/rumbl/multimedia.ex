defmodule Rumbl.Multimedia do
  ...
  def get_category_under_action_category() do
    # Video
    # |> join([v], u in assoc(v, :user))
    # |> join([v], c in assoc(v, :category))
    # |> on([v], v.category_id == ^category_id)
    #query = 
    transform_query()
    |> Repo.all()
    |> IO.inspect()
  ...
  defp transform_query() do
    category_id = 4
    from v in Video,
    join: u in assoc(v, :user),
    join: c in assoc(v, :category),
    where: v.category_id == ^category_id, select: %{
      url: v.url,
